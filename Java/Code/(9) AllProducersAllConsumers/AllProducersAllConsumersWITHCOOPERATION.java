 ...
 * @author Kamal
 */
public class AllProducersAllConsumersWITHCOOPERATION {
//------------- SOLUTION WITH COOPERATION BETWEEN PRODUCERS THEMSELVES AND CONSUMER THEMSELVES-------------------
            ...
            while (true) {
                ...
                if (turnProd) {
                    ...
                    if (q.size() == MAX_SIZE) {
                        turnProd = false;
                        full.signalAll();
                    } else {// IF I AM NOT THE LAST PRODUCER - Inform all other producers and wait a signal from another producer, then I can add another element if needed
                        addedEl.signalAll();
                        addedEl.await();
                    }
                }
            ...
            while (true) {
                ...
                if (!turnProd) {
                    ...
                    if (q.size() == 0) {
                        turnProd = true;
                        empty.signalAll();
                    } else {//If I AM NOT THE LAST CONSUMER - Inform other CONSUMERS and wait a signal from another, then I can remove another element
                        removedEl.signalAll();
                        removedEl.await();
                    }
